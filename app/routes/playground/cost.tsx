...
const stats = [
  { name: "Total Spend", value: "$1,245.82", change: "+12.5%", color: "indigo" },
...
const providerSpend = [
  { name: "fal.ai", spend: 742.50, color: "bg-indigo-500", icon: "✨" },
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <h2 className="text-xl font-bold text-zinc-100 font-heading">Cost Analysis</h2>
          <p className="text-sm text-zinc-500">Real-time spend tracking and resource utilization metrics.</p>
        </div>
          ...
          {["7D", "30D", "90D", "All"].map(p => (
            <button 
              ...
              className={cn(
                ...
                p === "30D" ? "bg-zinc-800 text-zinc-100 shadow-md border border-zinc-700/50" : "text-zinc-500 hover:text-zinc-300"
              )}
            >
              {p}
            </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat) => (
          <div key={stat.name} className="relative glass p-6 rounded-3xl overflow-hidden group hover:border-zinc-700 transition-all border-zinc-800/50">
            <div className={`absolute top-0 right-0 w-24 h-24 bg-${stat.color}-500/5 blur-3xl opacity-0 group-hover:opacity-100 transition-opacity`} />
            <div className="relative space-y-3">
              <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">{stat.name}</span>
              <div className="flex items-baseline justify-between">
                <h3 className="text-3xl font-black text-zinc-100 tracking-tighter">{stat.value}</h3>
                <span className={cn(
                  ...
                  {stat.change.startsWith('+') ? <ChevronUp className="size-2.5" /> : <ChevronDown className="size-2.5" />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 glass p-8 rounded-3xl space-y-8 border-zinc-800/50">
          <div className="space-y-1">
            <h3 className="text-lg font-bold text-zinc-200">Provider Breakdown</h3>
            <p className="text-xs text-zinc-500">Distribution of total spend per infrastructure partner.</p>
          </div>
                ...
                <div key={prov.name} className="space-y-2 group">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-semibold text-zinc-400 group-hover:text-zinc-200 transition-colors uppercase tracking-tight">{prov.name}</span>
                    </div>
                    <span className="text-sm font-mono text-zinc-100">${prov.spend.toFixed(2)}</span>
                  </div>
          ...
          <div className="pt-4 border-t border-zinc-800/50">
            <Button variant="ghost" className="w-full text-zinc-500 hover:text-zinc-300 text-xs font-bold uppercase tracking-widest">
              View Detailed Split
            </Button>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 glass rounded-3xl p-8 relative overflow-hidden border-zinc-800/50">
          <div className="space-y-1 relative z-10">
            <h3 className="text-lg font-bold text-zinc-200 uppercase tracking-tight">Daily Spend Trend</h3>
            <p className="text-xs text-zinc-500">Showing historical data for the past 30 days.</p>
          </div>
          ...
          <div className="mt-12 h-[260px] w-full relative flex items-end justify-between gap-1 group">
            ...
            {[12, 45, 32, 60, 48, 25, 35, 82, 55, 42, 38, 50, 65, 75, 45, 30, 42, 95, 60, 48, 55, 42, 68, 72, 58, 45, 32, 60, 48, 25].map((val, i) => (
              <div 
                key={i} 
